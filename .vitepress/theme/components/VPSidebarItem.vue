...
<template>
  <component :is="sectionTag" :class="classes" class="VPSidebarItem">
    <div
        v-if="item.text"
        :role="itemRole"
      ...
      <VPLink v-if="item.link" :href="item.link" :rel="item.rel" :tag="linkTag" :target="item.target" class="link">
        <component :is="textTag" class="text">
          <component :is="pageIcon" v-if="pageIcon" :size="16" class="sidebar-icon"/>
    ...
    <div v-if="item.items && item.items.length" class="items">
      <template v-if="depth < 5">
        <VPSidebarItem v-for="i in item.items" :key="i.text" :depth="depth + 1" :item="i"/>
      </template>
    </div>
  </component>
</template>
