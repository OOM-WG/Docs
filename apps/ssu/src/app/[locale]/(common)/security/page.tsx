...
export const generateMetadata = async ({ params }: PageProps<'/[locale]'>) => {
	...
	return pageMetadata(locale, 'security', getContent(locale).pages.security)
}
...
export default async ({ params }: PageProps<'/[locale]'>) => {
	...
	const ContentBody = mdx.pages.security

	return (
		<CommonPage eyebrow='Security' title={content.title} locale={locale} pathname='/security'>
			<ContentBody />
		</CommonPage>
	)
}