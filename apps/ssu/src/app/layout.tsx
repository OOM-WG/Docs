...
export default async ({ children }: Readonly<{ children: ReactNode }>) => {
	...
	return (
		<html lang='zh-Hans' suppressHydrationWarning>
			<head>
				{process.env.NODE_ENV === 'production' && <script defer src='https://☁️.ja7.top/🍥.js?ms=xf0q5hpejz' />}
							...
							<header className='border-base-content/10 bg-base-100/88 sticky top-0 z-40 border-b backdrop-blur'>
								<div className='navbar mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-12 xl:px-14'>
									...
									<nav className='navbar-center hidden gap-1 lg:flex'>
										{siteOrder.map(site => (
											<Link
												className='btn btn-ghost btn-sm hover:bg-primary/12'
												href={getSiteHref(site, '/', host)}
												key={site}>
												{siteConfigs[site].shortName ?? siteConfigs[site].name}
											</Link>
										))}
									</nav>
									<div className='navbar-end gap-2' data-nosnippet>
										<Link
											...
											href={getSiteHref('main', '/about', host)}>
											<Info size={16} />
