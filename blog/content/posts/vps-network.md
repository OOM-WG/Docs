...
tags: [VPS, 网络优化, 代理, BBR, sysctl, 内核优化, Linux, TCP]
category: 网络
---

最近突然想尝试自己搭建一条线路，看看会不会更好，遂购入了一台 VPS。 现在不想续费了，记录一下进行过的优化。本篇侧重于**网络优化**
...
> [线路与网络科普](https://meowvps.com/blog/route/)：常见各种线路和网络的科普，夯实网络理论基础
>
> [常见地区 VPS 推荐](https://meowvps.com/blog/resrec/)：不同地区 VPS 的选择逻辑与主流机房推荐
...
> [Digvps 测评](https://digvps.com/review)：提供详细的 VPS 性能、延迟及网络吞吐数据查询
>
> [NodeSeek 论坛](https://www.nodeseek.com/)：主打交流与最新信息（_⚠️ 交易与浏览时请谨防被骗_）
...
_在输出中，以支持的最大版本（如 `x86-64-v4` 或 `x86-64-v3`）为首选，请以你机器的实际输出为准_
...
_如果输出中包含 `xanmod` 字样，代表优化内核已成功激活_
...
:::tip[Nano 编辑器保存指南]
按 `Ctrl + O` 回车保存文件，再按 `Ctrl + X` 退出编辑器
:::
...
:::warning[关于 bpftune 的特别说明]
个人实测**不推荐**使用 `bpftune`，该工具可能无法判断内存的实际需求上限，导致不可控的运行状态
:::
